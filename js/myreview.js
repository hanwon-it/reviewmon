...
document.addEventListener("DOMContentLoaded", () => {
  ...
  review_list.onclick = async function (e) {
    ...
    if (e.target.classList.contains("btn_edit")) {
      ...
      edit_rating_row.addEventListener("mousemove", (ev) => {
        if (ev.target.classList.contains("star")) {
          ...
          edit_rating_row.querySelector(".edit_rating_value").textContent =
            value;
        }
      });
    ...
    if (e.target.classList.contains("btn_save")) {
      ...
      const new_rating =
        parseFloat(card.querySelector(".edit_rating_value").textContent) || 0;
      if (!new_content) {
        alert("내용을 입력해주세요.");
        return;
      }
  ...
  reviews.forEach((review) => {
    ...
    card.innerHTML = `
      <div class="review_card_top">
        ${
          review.movie_title && review.movie_title !== "undefined"
            ? `<span class="review_movie_title">[${review.movie_title}]</span>`
            : ""
        }
      </div>
...
function getEditableStarSVG(rating) {
  ...
  for (let i = 1; i <= 5; i++) {
    if (rating >= i) {
      ...
      stars += `<span class="star editable half" data-value="${
        i - 0.5
      }">&#9733;</span>`;
    } else {
      stars += `<span class="star editable" data-value="${i}">&#9734;</span>`;
