...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (movie_id) {
    ...
    fetch(`/reviews/movie/${movie_id}`)
      .then(res => res.json())
      .then(async data => {
        let filtered = Array.isArray(data) ? data.filter(r => String(r.movie_id) === String(movie_id)) : [];
        if (filtered.length > 0 && filtered[0].movie_title) {
          const h2 = document.querySelector('main.review_list_page h2');
          if (h2) h2.textContent = `[${filtered[0].movie_title}]의 리뷰 (${filtered.length})`;
        }
    ...
    fetch(`/reviews/user/${encodeURIComponent(nickname)}`, {
      ...
      .then(res => res.json())
      .then(async data => {
        let filtered = Array.isArray(data) ? data.filter(r => r.nickname === nickname) : [];
    ...
    card.innerHTML = `
      <div class="review_card_main" style="display:flex;flex-direction:column;gap:0.2em;">
        ${isUserPage
          ...
          : `<p class=\"review_author\"><a href=\"/reviewpage.html?nickname=${encodeURIComponent(review.nickname)}\" class=\"review_nickname_link\">${review.nickname}</a></p>`}
